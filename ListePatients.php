...
if (isset($_GET["search"])) {
    $nom = $_GET["nom"];
    $prenom = $_GET["prenom"];
    $requette = "select * from patients  where nom like '%$nom%'and prenom like '%$prenom%' order by nom limit $offset,$limit";
} else {
    $requette = "select * from patients order by nom limit $offset,$limit";
}
...
<body>
    ...
    <table>
        ...
        while ($ligne = mysqli_fetch_assoc($execute)) {
            ...
            $numpat = $ligne["numpat"];
            echo " <tr><td>$ligne[numpat]</td><td>$ligne[nom]</td><td>$ligne[prenom]</td><td>$ligne[ville]</td><td>$ligne[sexe]</td><td><a href='modifier.php?identifiant=$numpat'><img src='modif.png' alt='modifier' width='15px'></a></td><td><a href='supprimer.php?identifiant=$numpat'><img src='sup.png' alt='Supprimer' width='15px'></a></td></tr>";
        }
        ?>
